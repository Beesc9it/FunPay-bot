...
- Собственный Python-пакет FunPayAPI, который никак не привязан к FunPay-bot.
...
7. Заходим в папку `configs` и в `_main.cfg`
   ...
   Расширение для получения `golden_key голден кей получаем на сайте funpay` (НУЖНО БЫТЬ АВТОРИЗОВАННЫМ НА САЙТЕ):
    ...
    Ваш юзер агент можно получить тут:
       ...
       https://whatmyuseragent.com/ (нажмите на `copy`)
  ...
  secretKey - это  тайный пароль который позволит использовать бота только вам  (нужно будет ввести при первом запуске бота) 

    можно ввести в это поле что угодно (пример:password123)
